...
class AcrobotDMControlWrapper(DMControlWrapper):
                ...
                'agent': swm_space.Dict(
                    {
                        ...
                        'upper_arm_locked': swm_space.Discrete(
                            2, init_value=1
                        ),
                    }
                ),
    ...
    def modify_mjcf_model(self, mjcf_model):
        ...
        base = (
            upper_arm_geom.density
            if upper_arm_geom.density is not None
            else 1000.0
        )
        ...
        base = (
            lower_arm_geom.density
            if lower_arm_geom.density is not None
            else 1000.0
        )
        desired_density = float(
            np.asarray(
                self.variation_space['agent']['lower_arm_density'].value
            ).reshape(-1)[0]
        )
        if not np.allclose(base, desired_density):
            mass_changed = True
        ...
        assert target_site is not None, "Expected geom named 'target'"
