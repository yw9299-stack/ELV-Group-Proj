...
class SceneEnv(ManipSpaceEnv):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._cube_colors = np.array(
            [self._colors['red'], self._colors['blue']]
        )
        ...
        self.variation_space = swm_spaces.Dict(
            {
                ...
                'lock_color': swm_spaces.Box(
                    low=0.0,
                    high=1.0,
                    shape=(2, 3),
                    dtype=np.float64,
                    init_value=np.array(
                        [self._colors['red'][:3], self._colors['red'][:3]]
                    ),
                ),
    ...
    def add_objects(self, arena_mjcf):
        ...
        button_mjcf = mjcf.from_path(
            (self._desc_dir / 'buttons.xml').as_posix()
        )
        arena_mjcf.include_copy(button_mjcf)
        drawer_mjcf = mjcf.from_path(
            (self._desc_dir / 'drawer.xml').as_posix()
        )
        arena_mjcf.include_copy(drawer_mjcf)
        window_mjcf = mjcf.from_path(
            (self._desc_dir / 'window.xml').as_posix()
        )
        ...
        for i in range(self._num_buttons):
            self._button_geoms_list.append(
                [button_mjcf.find('geom', f'btngeom_{i}')]
            )
    ...
    def post_compilation_objects(self):
        ...
        self._drawer_target_site_id = self._model.site(
            'drawer_handle_center_target'
        ).id

        self._window_site_id = self._model.site('window_handle_center').id
        self._window_target_site_id = self._model.site(
    ...
    def _apply_button_states(self):
        ...
        if self._cur_button_states[0] == 0:
            ...
            self._model.material('drawer_handle').rgba[:3] = (
                self.variation_space['lock_color'].value[0]
            )
        ...
        if self._cur_button_states[1] == 0:
            ...
            self._model.material('window_handle').rgba[:3] = (
                self.variation_space['lock_color'].value[1]
            )
    ...
    def initialize_episode(self):
        ...
        if self._mode == 'data_collection':
            ...
            self._data.joint('drawer_slide').qpos[0] = self.np_random.uniform(
                -0.16, 0
            )
            self._data.joint('window_slide').qpos[0] = self.np_random.uniform(
                0, 0.2
            )
            ...
            init_block_xyzs = self.cur_task_info['init']['block_xyzs'].copy()[
                block_permutation
            ]
            goal_block_xyzs = self.cur_task_info['goal']['block_xyzs'].copy()[
                block_permutation
            ]
            # Get the current task info for the other objects.
            init_button_states = self.cur_task_info['init'][
                'button_states'
            ].copy()
            goal_button_states = self.cur_task_info['goal'][
                'button_states'
            ].copy()
            ...
            self._model.site('drawer_handle_center_target').pos[1] = (
                goal_drawer_pos
            )
            ...
            self._model.site('window_handle_center_target').pos[0] = (
                goal_window_pos
            )
    ...
    def set_new_target(self, return_info=True, p_stack=0.5):
        ...
        self._target_task = self.np_random.choice(
            ['cube', 'button', 'drawer', 'window'], p=probs
        )
        ...
        if self._target_task == 'cube':
            ...
            put_in_drawer = (
                ...
                and self.np_random.uniform() < p_put_in_drawer
            )
            ...
            if put_in_drawer:
                ...
                tar_pos[:2] = tar_pos[:2] + self.np_random.uniform(
                    -0.005, 0.005, size=2
                )
            elif stack:
                ...
                block_pos = self._data.joint(f'object_joint_{block_idx}').qpos[
                    :3
                ]
                tar_pos = np.array(
            ...
            if (
                self._data.joint('drawer_slide').qpos[0] >= -0.08
            ):  # Drawer closed.
            ...
            if (
                self._data.joint('window_slide').qpos[0] <= 0.1
            ):  # Window closed.
    ...
    def _compute_successes(self):
        ...
        drawer_success = (
            np.abs(
                self._data.joint('drawer_slide').qpos[0]
                - self._target_drawer_pos
            )
            <= 0.04
        )
        window_success = (
            np.abs(
                self._data.joint('window_slide').qpos[0]
                - self._target_window_pos
            )
            <= 0.04
        )
    ...
    def set_cube_target_pos(self, cube_id, target_pos, target_quat=None):
        ...
        if cube_id < 0 or cube_id >= num_target_pos:
            raise ValueError(
                f'cude_id out of range (maximum {num_target_pos - 1})'
            )
    ...
    def post_step(self):
        ...
        if self._mode == 'task':
            ...
            for i in range(self._num_cubes):
                ...
                if np.any(
                    obj_pos <= self._workspace_bounds[0] - 0.2
                ) or np.any(obj_pos >= self._workspace_bounds[1] + 0.2):
                    is_healthy = False
                    break
    ...
    def add_object_info(self, ob_info):
        ...
        # Drawer states.
        ob_info['privileged/drawer_pos'] = self._data.joint(
            'drawer_slide'
        ).qpos.copy()
        ob_info['privileged/drawer_vel'] = self._data.joint(
            'drawer_slide'
        ).qvel.copy()
        ...
        # Window states.
        ob_info['privileged/window_pos'] = self._data.joint(
            'window_slide'
        ).qpos.copy()
        ob_info['privileged/window_vel'] = self._data.joint(
            'window_slide'
        ).qvel.copy()
        ...
        if self._mode == 'data_collection':
            ...
            ob_info['privileged/target_drawer_handle_pos'] = (
                self._data.site_xpos[self._drawer_target_site_id].copy()
            )
            ...
            ob_info['privileged/target_window_handle_pos'] = (
                self._data.site_xpos[self._window_target_site_id].copy()
            )
