...
def run(cfg):
    ...
    checkpoint_dir = Path(cfg.get("checkpoint_dir", None) or cache_dir)
    checkpoint_dir.mkdir(parents=True, exist_ok=True)
    resume_ckpt_path = cfg.get("resume_ckpt_path", None) or str(
        checkpoint_dir / f"{cfg.output_model_name}_weights.ckpt"
    )
