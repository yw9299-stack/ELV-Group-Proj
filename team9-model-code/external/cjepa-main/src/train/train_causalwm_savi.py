...
import wandb
from src.world_models.dinowm_causal_savi import CausalWM_Savi
...
def get_world_model(cfg):
    """Build world model: frozen savi encoder + masked slot predictor."""
...
# Main Entry Point
# ============================================================================
@hydra.main(version_base=None, config_path="../../configs", config_name="config_train_causal_savi")
def run(cfg):
    ...
    if cfg.get("monitor_rankme", False):
        if "clevrer" in cfg.dataset_name:
            target_shape = cfg.savi.NUM_SLOTS * cfg.savi.SLOT_DIM  # S * D flattened
        elif "pusht" in cfg.dataset_name:
            target_shape = cfg.savi.NUM_SLOTS * (cfg.savi.SLOT_DIM + cfg.dinowm.action_embed_dim + cfg.dinowm.proprio_embed_dim)  # S * D flattened
